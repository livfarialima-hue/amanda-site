...
## 12 de setembro de 2026 — roteamento por intenção e correção de relevância dos RSAs secundários

- **Status:** publicado e verificado em Google Ads, Apps Script v149, branch canônico, Netlify e na mesma projeção do Plano no Drive.
- **Área:** `S_BR_SP_CIRURGIA_MAMA`, `S_BR_SP_CONTORNO_CORPORAL`, dois grupos genéricos transitórios, três novos grupos específicos de preço, oito palavras existentes, seis negativas exatas já aplicadas, três RSAs novos, dois RSAs corrigidos, agregado `Google_Rotas` e rotina de revisão somente leitura.
- **Evidência:** a releitura ao vivo encontrou as duas campanhas ativas em R$ 8/dia, ainda sem impressão, clique ou gasto. Os dois grupos genéricos misturavam procedimentos e seus RSAs, além dos RSAs de `AG_MASTOPEXIA` e `AG_ABDOMINOPLASTIA`, apareciam como `Ruim`. Depois da intervenção, os três grupos específicos, oito palavras e três RSAs novos ficaram ativos e qualificados; os dois RSAs existentes passaram a `Excelente`; e os genéricos foram pausados somente após a releitura dos substitutos. Cinco uploads concluíram sem erro (`27885666172234525`, `8370800241093983490`, `776172141302390487`, `8536118842992338984` e `3397105701539708642`). O Apps Script canônico avançou para v149 no mesmo deployment, com SHA-256 normalizado `d846de34a0a289fb11cd7a27794b1dd9693649d67d48bfa2db110e01d090e7c7`, GET HTTP 200 e segredo sintético inválido recusado sem escrita.
- **Mudança publicada:** as seis negativas já aplicadas foram preservadas sem nova escrita; `AG_MASTOPEXIA_PRECO`, `AG_PROTESE_MAMA_PRECO` e `AG_LIPOASPIRACAO_PRECO` receberam RSAs específicos nos dois guias gerais e as oito palavras com a mesma correspondência. Após a prévia e a releitura, `AG_CIRURGIA_MAMA_PRECO` e `AG_CONTORNO_CORPORAL_PRECO` foram pausados sem exclusão. Somente os RSAs existentes de `AG_MASTOPEXIA` e `AG_ABDOMINOPLASTIA` foram editados. Campanhas, R$ 8/dia em cada uma, lances, meta qualificada, redes, geografia, idades, dispositivos, agenda, sitelinks e os outros três RSAs de procedimento ficaram iguais.
...
- **Mensuração e lógica:** `ag_cirurgia_mama_preco` e `ag_contorno_corporal_preco` permanecem apenas para ler o intervalo transitório; `ag_mastopexia_preco`, `ag_protese_mama_preco` e `ag_lipoaspiracao_preco` estão registrados e ativos. Os aliases genéricos não inventam procedimento; os grupos específicos preservam procedimento, intenção e landing. A qualificação continua exigindo manifestação pessoal. O consumidor Netlify para reconhecer os sufixos `_preco` no contexto do webhook foi publicado no commit preservador `64ae2c598816c033b4e2a995d064adddf80de54a`, deploy `6aa5e56c65ed3d0008cebf97`, e respondeu em modo `active` com o mesmo estado de saúde no domínio e na URL imutável. O deploy posterior `6aa5e68f41e08e0008a76b33`, commit descendente `de53af57c2d17d2fb5912379dc05f38f024dc7de`, preservou as 12 funções desse baseline; a mesma projeção do Plano no Drive foi relida depois do fechamento documental.
- **Hipótese:** grupos e RSAs específicos, com as expressões reais do próprio grupo, aumentam continuidade e permitem comparar intenção de preço com intenção de procedimento sem elevar gasto ou recorrer a texto artificial.
...
- **Regra para reverter:** restaurar somente as palavras ao grupo de origem, retirar a negativa correspondente e restaurar o texto anterior do RSA afetado diante de destino/política/atribuição incorretos ou piora downstream consistente; não reverter por oscilação isolada de CTR, CPC ou pontuação do anúncio.
...
- **Mudança publicada:** dois pilotos de Pesquisa, R$ 8/dia cada, até 12/10/2026; São Paulo por presença, português, todos os dispositivos/idades incluindo desconhecida, parceiros e Display desligados, Maximizar cliques e meta personalizada de lead qualificado. Mama separa mastopexia, redução e prótese; corpo separa abdominoplastia e lipoaspiração. Foram publicados cinco grupos, 13 palavras exatas/frase aprovadas, cinco RSAs e 20 sitelinks contextuais com destinos específicos. As campanhas receberam os IDs `24239348529` e `24239348532` e foram relidas como `Ativada` e `Qualificada (aprendizado)`.
...
- **Recibos:** campanhas `8304615862134805016`; grupos `1035226498139761048`; palavras aprovadas `4941378120393558080`; RSAs `6913781089229317932`; sitelinks `1668147931032527165`; ativação `5196337282830450399`. Todos os arquivos aplicados terminaram sem erro. A prévia inicial de palavras encontrou 24 bloqueios de política e não foi aplicada; as 13 palavras conformes foram reenviadas separadamente, sem pedir exceção nem contornar a política.
...
- **Status:** publicado e verificado em site, Apps Script, planilha, Google Ads e Search Console em 12/09/2026.
...
- **Mudança publicada:** novas páginas específicas de preço para blefaroplastia e lifting cervical; sitelink `Preço da Blefaroplastia` no nível da campanha BLEF e `Preço do Lifting Cervical` somente em `AG_CERVICOPLASTIA`; teste único do rótulo/local do CTA fixo móvel no guia de lifting; aba anônima `Google_Rotas` por grupo, landing e CTA, sem alterar `Agregados` v2. A palavra exata adicional `[cirurgia de blefaroplastia preço]` foi barrada na prévia pela política de saúde em publicidade personalizada e não foi aplicada nem forçada; a campanha preserva `[quanto custa uma blefaroplastia]` já aprovada e passa a acessar o guia também pelo sitelink.
- **Schema reconciliado:** a v146 materializou a extensão aditiva; a primeira execução por célula atingiu seis minutos e foi substituída pela v147 em lote. A aplicação final terminou em 24 segundos e a publicação dos agregados em 5 segundos, com os três IDs canônicos confirmados e o deployment preservado. O histórico anterior permanece N/D quando a dimensão não foi observada; nenhuma rota foi inferida.
- **Item 7, reavaliado após a reclassificação e o pós-voo:** `AJUSTES_GOOGLE_ADS` continua sem linha pendente. `Google Ads - Conversões` tem seis eventos qualificados marcados para envio, `IMPORT_GOOGLE_ADS` contém as mesmas seis transações e todas correspondem por `Opportunity ID` a uma oportunidade qualificada em `Funil Comercial`. A conexão diária `LEADS` concluiu a execução de 12/09 às 05:05 com 6 linhas e 0 erros e mantém a próxima execução programada para 13/09 às 05:05. A importação direta acrescenta qualificados; a rota de ajustes permanece disponível para correção idempotente futura sem somar as duas fontes.
...
- **Guardrails:** páginas vivas e verificadas antes de apontar o Ads; zero faixa pública; BLEF sem promessa de faixa automática; cervical com referência privada contextualizada; os R$ 87/dia das seis campanhas anteriores, seus lances, metas, RSAs, negativas, públicos e geografia preservados; o incremento MAMA/CORP fica registrado separadamente; nenhum dado pessoal no agregado.
- **Comprovantes:** commit funcional `dd1bd1d18c79eb216626e36adb49de895d27a0f6`; deploy funcional Netlify `6aa589ce092f3b0008413443`; commit documental `cfee5263ddfc09681c381b08e9498622904b3ced` no deploy atual `6aa59666f301b80008ffb340`, `production` e `ready`. O domínio e a URL imutável do deploy atual responderam HTTP 200 com conteúdo idêntico nas quatro páginas de custo e no sitemap. Os dois sitelinks foram aplicados pelo recibo `4197694077316228599`, ficaram ativos e em análise, com URL e nível corretos. O mesmo `sitemap.xml` foi reenviado; o Search Console passou de 44 para 46 páginas processadas, com envio e leitura em 12/09/2026. Apps Script atual na v147.
...
- **Mudança publicada:** reabertos exclusivamente os dois objetos canônicos concluídos, elevando cada orçamento vitalício de R$ 300 para R$ 900 e levando a data final dos dois conjuntos de 01/09/2026 12:00 para 11/10/2026 23:59, GMT-3. Como aproximadamente R$ 300 já haviam sido gastos em cada campanha, isso libera cerca de R$ 600 novos por campanha por 30 dias, mantendo o padrão de R$ 20/dia e um teto novo conjunto de R$ 1.200.
...
- **Recibo:** no resumo final foram selecionadas 2 de 4 campanhas, 2 de 4 conjuntos e 0 de 3 anúncios; o contador caiu de 11 para 7. `M26C01W` e `M26F01W` e seus conjuntos foram relidos como `Ativo`, com R$ 900 por campanha; nenhuma outra entidade foi publicada.
- **Revisão:** saúde em 13/09; negócio em 19/09, 26/09 e 12/10; latência em 19/10, contados da publicação real de 12/09.
...
- **Status:** publicada e verificada na Apps Script v142 e no Netlify, ainda desligada. Passaram 1.263/1.263 testes, arquitetura, escopo exato, build de 180 arquivos, inspeção das 44 URLs e `git diff --check`. Nenhuma função operacional, trigger, propriedade de ativação, planilha, fila ou mensagem real foi executado.
...
- **Mudança publicada default-off:** uma primeira retomada automática por modelo, entre 24 e 48 horas, apenas para duas mensagens exatas — interesse genérico em procedimento conhecido e resposta inicial da Bruna com abertura, procedimento e pergunta permitidos, ainda sem resposta. O contrato é repetido no Apps Script e no endpoint; duas flags independentes, preflight autenticado, instante de ativação e marcador no plano impedem ativação por publicação e qualquer backlog. A ativação reinstala o planejador diário e o processador antes de ligar as flags. Segunda retomada, preço, agenda, pós-consulta, contexto clínico ou sensível, nova atividade, takeover, opt-out, pausa, promessa e ambiguidade permanecem humanos.
- **Recibos:** commit funcional aprovado `3d022332a6f1f2dbd7729c45c3083b1696172812`; deployment Apps Script canônico preservado e atualizado para v142; Netlify `6aa536c2f301b80008efe5e6`, `ready`, `production` e no mesmo commit. Os dois arquivos vivos coincidiram com o candidato; domínio e URL imutável coincidiram; token inválido não aplicou decisão; acesso sem autenticação foi recusado. As duas flags e a variável Netlify continuaram ausentes.
- **Prontidão para ativação:** às 09:09, o diagnóstico autenticado somente leitura confirmou modelo configurado, endpoint HTTP 200 saudável, gatilhos 1/1 e zero plano programado, automático por modelo, legado ou manual aprovado. Estado `PRONTO PARA ATIVAR`, ainda default-off e aguardando nova autorização explícita.
...
- **Status:** publicado e verificado no site, Google Ads e Plano Executivo canônico.
...
- **Mudança publicada:** retiradas apenas as associações erradas de sitelink, recursos adulto/infantil separados por grupo, negativa de frase `"lobuloplastia"` adicionada, resposta de valor sem faixa cirúrgica publicada nas duas páginas e `otomodelação` esclarecida sem presumir técnica. Permaneceram cinco CTAs, OT01/OT02 e tracking em cada página.
- **Recibos:** commit funcional `b845db8fb2cce7889abc6ae3e8334261ba3661da`; Netlify `6a99862d5c80270008ebf19b`, `ready`, `production` e com o mesmo `commit_ref`; páginas pública e imutável HTTP 200. A associação de conta `Blefaroplastia` foi pausada, sem exclusão, e o recurso foi associado diretamente à campanha BLEF com a URL correta. A associação cervical foi pausada somente em OTO e permaneceu ativa em CERV. No infantil entraram sete sitelinks de grupo; no adulto, dois destinos foram corrigidos e dois sitelinks foram adicionados. O pós-teste confirmou R$ 15/dia, Maximizar cliques, meta específica, campanha e grupos ativos, RSAs e nove frases de destaque preservados. Apps Script v141 ficou intocado.
...
- **Status:** publicado e verificado; monitoramento em 24 horas, 7 dias e 14 dias.
...
- **Mudança executada:** um sitelink de campanha para o guia de preço; resposta curta e distinção cirúrgica visível na página principal; distinção entre lifting e abordagens sem cirurgia também no guia de preço; autoria, revisão, `MedicalWebPage`/`MedicalProcedure` e sitemap atualizados sem alterar CTAs.
...
- **Comprovantes:** commit funcional `fe9b3348a25b4a78b5dc4853a78563e774322999`; Netlify `6a96a5f48f881800083c1744`, em produção e no mesmo `commit_ref`; sitelink `Preço do Lifting Facial` relido como ativado e em análise, com a URL final exata; domínio e URL imutável HTTP 200 e com hashes iguais; seis CTAs, o mesmo destino WhatsApp e seis atributos de tracking por página; canonical, dados estruturados e sitemap confirmados.
- **Data de revisão:** integridade em 2 de setembro de 2026; negócio em 8 e 15 de setembro de 2026.
...
### 12 de setembro de 2026 — comunicação Google Ads com segurança concreta e escolha da cirurgiã

- **Status:** aplicada e relida na conta `995-334-4486`; recursos novos sujeitos à análise do Google. Nenhuma publicação de site por esta tarefa.
- **Autorização e motivo:** Daniel pediu revisão ampla das campanhas ativas, por considerar a linguagem genérica e artificial, e definiu 12/09 como novo marco editorial. Acrescentou a prioridade da segurança, a cardiologia na própria clínica e a seleção criteriosa dos anestesistas.
- **Evidência:** oito campanhas e 17 RSAs ativos; recursos da conta excessivamente faciais, slogans categóricos e descrições de formação pouco informativas. A página de segurança sustenta cardiologia integrada e formação USP do cardiologista; Daniel confirmou o critério de escolha da anestesia. A condição comercial de cardiologia incluída no pacote foi reconciliada com a seção 29 do Norte, sem virar promoção no anúncio.
- **Mudança:** banco final com 236 títulos e 68 descrições, identificação médica fixada na descrição 1; 29 associações de destaques e oito modelos de sitelinks em 12 associações. Segurança é apresentada por preparo, comunicação da equipe e formação corretamente atribuída. Naturalidade é objetivo do planejamento; a publicidade infantil não presume cardiologia pediátrica. Preço continua próprio da intenção de custo, sem faixas inventadas ou gancho de menor preço.
- **Hipótese:** pessoas que comparam cirurgiões para cirurgia particular poderão reconhecer melhor o valor do cuidado da Amanda e avançar para contato válido, qualificação e consulta; ganho de conversão não foi demonstrado por esta execução.
- **Métrica principal:** contatos válidos, qualificados, consultas agendadas/realizadas e custo por etapa. CTR, cliques e qualidade do anúncio são diagnósticos. Mudanças simultâneas impedem teste causal isolado da redação.
- **Guardrails:** site, Meta, bot, mensagens, Apps Script, CRM, verba, lances, público, palavras, metas, URLs, caminhos e rastreamento não foram modificados nesta tarefa; campanhas pausadas preservadas. Sem promessa de risco zero, resultado garantido, superioridade ou endosso da USP.
- **Verificação:** 17 anúncios ativados/qualificados relidos na listagem; 63 destaques e 76 sitelinks conferidos; 41 associações alteradas confirmadas. Nove formulários de RSA tiveram releitura adicional integral e equivalência exata ao banco; backup anterior recuperado para 16/17, com lacuna documentada. Recibo: `REVISAO-COMUNICACAO-GOOGLE-ADS-2026-09-12.md`; bancos e JSON de verificação em `auditorias/google-ads-comunicacao-2026-09-12/`, fora do artefato público.
...
- **Estado:** publicado e verificado em 13/09/2026 às 10h22 BRT, commit `77433528d0247c46f5482db29932094e0f534783`, Netlify `6aa6a37365ed3d0008e3d1cc`. Reconciliado sobre `fcae9e42`; 1.468/1.468 testes, 54/54 páginas, 63/63 sondas e 12 funções/cinco programações idênticas. Apps Script151 e aniversário manual preservados. Nenhuma mensagem de teste ou alteração de Ads.
...
- **Revisão e reversão:** integridade imediata/24h; D+7 em 20/09, D+14 em 27/09 e D+30 em 13/10, por Daniel/equipe, sem automação. Corrigir inexatidão ou regressão; avaliar contatos qualificados e consultas somente com classificação e maturação suficientes. Predecessor Netlify `6aa6998a46297000089e1e5d`, sem reversão de Apps Script/dados; se houver publicação posterior, reverter apenas o delta editorial.

### 13 de setembro de 2026 — diferenciação e continuidade da otoplastia Google

- **Pedido:** Daniel relatou nenhuma consulta desde a criação, pediu estudar causas, explicitar diferenciais da Amanda, fazer modificações e agendar revisão breve, sem aumentar orçamento.
- **Evidência:** 357 cliques/R$788,89 até12/09 no histórico Google; três contatos canônicos na coorte90dias, sem consulta e sem classificação, dois resumos com objeção Preço e um sem resposta. Seis conversões históricas de interface não são seis consultas. Adulto92,37%gasto; infantil29cliques. Mudanças de anúncios12/09 e site13/09 limitam causalidade.
- **Decisão e hipótese:** explicitar Cirurgiã Plástica pela Unicamp/RQE e sua aplicação a formato, dobras, projeção, cicatrizes e recuperação; consulta com a própria cirurgiã e R$500 visíveis; criança ouvida. Maior clareza pode melhorar continuidade, ainda sem ganho demonstrado. Sem alegações falsas contra profissões: matriz ORL inclui otoplastia e normas CFO mudaram em2026.
- **Escopo:** dois RSAs e duas páginas G26OTO; R$15/dia e demais orçamentos/lances/segmentação/metas/palavras/tracking preservados. Sem alteração de bot, Apps Script ou mensagens. Release editorial77433528 reconciliado antes de publicar. Estado e horários reais no recibo auditorias/otoplastia-estrategia-2026-09-13/PUBLICACAO.json.
- **Métrica/data:** integridade14/09; leitura breve16/09 às9h BRT; D+7 em21/09 e D+14 em28/09. Contatos identificados/válidos/qualificados e consultas únicas da mesma coorte; classificação ausente é N/D. Automação vinculada à tarefa, sem efeitos externos automáticos e silenciosa quando não há mudança ou marco.
- **Manter/reverter:** zero contato em7dias com≥50cliques ou≥R$105 exige proposta de correção ou pausa para Daniel, nunca aumentar verba. Pouca amostra é inconclusiva. Corrigir inexatidão, reprovação ou caminho quebrado; reverter apenas delta deste pacote, preservando produção paralela. Norte seção35 é a decisão vigente.
- **Aplicação verificada:** site em 2026-09-13T13:57:01.241Z, commit030c57da84878f0910111d5e8de2a71b91538e82/deploy6aa6ab8ec7bf45000934fe35; RSAs816612405034/816612778213 relidos às14:03/14:05UTC, textos e descrição1 fixada exatos. R$15/dia preservados. Próxima leitura16/09 às9h BRT; nenhuma mensagem/consulta/conversão de teste.

### 13 de setembro de 2026 — reconciliação LEADS × conversas exportadas

- **Status:** aplicada e verificada na planilha canônica; aceite da linha nova pelo Google Ads pendente da próxima execução diária da conexão.
- **Pedido e evidência:** Daniel pediu cruzar a LEADS com as conversas exportadas no Drive, corrigir oportunidades qualificáveis e preparar para o Google apenas os eventos pertinentes. Foram consolidadas 377 exportações em 289 conversas únicas; a coorte recente teve 140 conversas, 132 correspondências diretas e oito resoluções manuais, sem copiar PII para o repositório.
- **Decisão:** uma oportunidade passou de `Novo` para `Qualificado` porque havia manifestação pessoal posterior ao prefill e interesse concreto em avaliação. Prefill isolado, pergunta somente de preço/localização, pesquisa genérica e mídia sem texto continuaram em `Novo`; fases superiores foram preservadas.
- **Aplicação:** a rotina auditada sincronizou LEADS, CRM, funil e evento de fase. A oportunidade elegível recebeu `Enviar ao Google Ads? = Sim`, ação `Lead qualificado GCLID`, valor 1, BRL e transação idempotente. O conjunto pronto passou de seis para sete linhas, com zero problemas e zero duplicidades.
