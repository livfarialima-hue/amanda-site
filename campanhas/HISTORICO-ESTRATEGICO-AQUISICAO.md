...
## 12 de setembro de 2026 — candidato de roteamento por intenção e correção de relevância dos RSAs secundários

- **Status:** refinamento local em reconciliação; uma aplicação paralela dos dois grupos genéricos foi incorporada, mas a publicação do desenho específico depende de novo commit e autorização exata.
...
- **Evidência:** a releitura ao vivo das 19h encontrou as duas campanhas ativas em R$ 8/dia, ainda sem impressão, clique ou gasto. Uma execução paralela já havia criado `AG_CIRURGIA_MAMA_PRECO` e `AG_CONTORNO_CORPORAL_PRECO`, movido para eles as oito palavras e aplicado as seis negativas exatas nos grupos de origem. Os dois RSAs genéricos e os RSAs de `AG_MASTOPEXIA` e `AG_ABDOMINOPLASTIA` aparecem como `Ruim`; os genéricos misturam procedimentos e os dois originais têm baixa presença das palavras ativas. O Apps Script canônico já está na v148 com os dois códigos transitórios. Os guias de custo seguem publicados, HTTP 200, rastreáveis e adequados à intenção.
- **Mudança candidata:** preservar as seis negativas já aplicadas; criar `AG_MASTOPEXIA_PRECO`, `AG_PROTESE_MAMA_PRECO` e `AG_LIPOASPIRACAO_PRECO`, apontar seus RSAs específicos aos dois guias gerais e recriar neles as oito palavras com a mesma correspondência. Somente depois da prévia e da releitura dos três grupos novos, pausar de forma reversível os dois grupos genéricos transitórios. Editar apenas os RSAs atuais de `AG_MASTOPEXIA` e `AG_ABDOMINOPLASTIA`; os dois RSAs genéricos deixam de concorrer ao serem pausados. Preservar campanhas, R$ 8/dia em cada uma, lances, meta qualificada, redes, geografia, idades, dispositivos, agenda, sitelinks e os outros três RSAs de procedimento.
...
- **Mensuração e lógica:** manter `ag_cirurgia_mama_preco` e `ag_contorno_corporal_preco` apenas para ler o intervalo transitório e registrar `ag_mastopexia_preco`, `ag_protese_mama_preco` e `ag_lipoaspiracao_preco` antes de ativá-los. Os aliases genéricos não inventam procedimento; os grupos específicos preservam procedimento, intenção e landing. A qualificação continua exigindo manifestação pessoal.
