...
- **Item 7, reavaliado após a reclassificação:** a rota `AJUSTES_GOOGLE_ADS` mostrou somente os dois recibos `confirmed_not_found` e nenhuma linha pendente, enquanto a conexão direta importou seis linhas com zero erro. A programação de ajustes permanece ativa: a importação direta acrescenta qualificados, mas não retrai uma conversão já importada se a fase for corrigida depois. Com fila vazia, a rotina é inócua e preserva a correção idempotente futura.
