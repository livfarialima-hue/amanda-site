...
- **Área:** `S_BR_SP_CIRURGIA_MAMA`, `S_BR_SP_CONTORNO_CORPORAL`, três novos grupos de preço, oito palavras existentes, seis negativas exatas, três RSAs novos, dois RSAs corrigidos, agregado `Google_Rotas` e rotina de revisão somente leitura.
- **Evidência:** a releitura ao vivo encontrou as duas campanhas ativas, ainda sem impressão, clique ou gasto. Cinco palavras de preço/valor estavam misturadas aos grupos de mastopexia/prótese e três ao grupo de lipoaspiração, todas com destino na página de procedimento. Na tabela, `AG_MASTOPEXIA` e `AG_ABDOMINOPLASTIA` apareciam como `Ruim`; o editor confirmou 15/15 títulos, apenas 3/4 descrições salvas e baixa presença das palavras ativas. Os dois guias gerais de custo já estavam publicados, HTTP 200, rastreáveis e com comunicação direta sobre composição do orçamento, consulta e pagamento. O piloto anterior de lifting facial mostrou que intenção de preço pode consumir cliques sem downstream, portanto a separação é um teste controlado, não promessa de ganho.
- **Mudança candidata:** criar `AG_MASTOPEXIA_PRECO`, `AG_PROTESE_MAMA_PRECO` e `AG_LIPOASPIRACAO_PRECO`, apontar seus RSAs específicos aos dois guias gerais, recriar neles as oito palavras com a mesma correspondência e pausar as oito originais somente depois da prévia sem erro. Acrescentar seis negativas exatas de roteamento nos grupos de origem. Editar somente os RSAs atuais de `AG_MASTOPEXIA` e `AG_ABDOMINOPLASTIA`: completar quatro descrições e substituir títulos genéricos por linguagem natural que corresponda às palavras ativas. Preservar campanhas, R$ 8/dia em cada uma, lances, meta qualificada, redes, geografia, idades, dispositivos, agenda, sitelinks e os outros três RSAs atuais.
...
- **Mensuração e lógica:** registrar `ag_mastopexia_preco`, `ag_protese_mama_preco` e `ag_lipoaspiracao_preco` antes de ativá-los. O agregado aceita apenas grupo e landing exatos, preserva procedimento e intenção de preço e continua exigindo manifestação pessoal para qualificação. A rotina somente leitura passa a reconhecer os procedimentos das campanhas e o orçamento total vigente de R$ 103/dia.
