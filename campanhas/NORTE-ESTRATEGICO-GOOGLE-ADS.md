...
**Estrutura e destinos:** manter intenção de procedimento em `AG_MASTOPEXIA` → `/mastopexia/`, `AG_MAMOPLASTIA_REDUTORA` → `/mamoplastia-redutora/`, `AG_PROTESE_MAMA` → `/protese-de-mama/`, `AG_ABDOMINOPLASTIA` → `/abdominoplastia/` e `AG_LIPOASPIRACAO` → `/lipoaspiracao/`. A alteração autorizada de 12/09/2026 cria `AG_MASTOPEXIA_PRECO` e `AG_PROTESE_MAMA_PRECO` → `/conteudos/quanto-custa-cirurgia-plastica-mama-sao-paulo/` e `AG_LIPOASPIRACAO_PRECO` → `/conteudos/quanto-custa-cirurgia-plastica-corporal-sao-paulo/`, move para eles somente as oito palavras ativas de preço/valor e instala seis negativas exatas nos grupos de origem. A divisão por procedimento substitui o desenho anterior de dois grupos regionais genéricos porque mantém cada RSA coerente com suas próprias palavras e permite leitura mais precisa do funil. Até existir recibo de aplicação, os cinco grupos publicados continuam sendo o estado vivo; não declarar os três novos grupos como ativos por antecipação.
...
**Mensuração:** `{_camp}` mantém `G26MAMA` ou `G26CORP`; `{_ag}` passa a distinguir oito grupos depois da publicação coordenada do registro. Marcações `ag_mastopexia_preco`, `ag_protese_mama_preco` e `ag_lipoaspiracao_preco` preservam procedimento, intenção de preço e landing sem depender de um rótulo regional genérico. Marcação automática e `GCLID`/`GBRAID`/`WBRAID` permanecem preservados. A jornada first-party registra campanha, grupo, landing e CTA, e LEADS/CRM resolve apenas códigos e rotas exatas. Lead qualificado só entra na importação padrão com manifestação pessoal, fase canônica e exatamente um click ID elegível; clique, prefill e pergunta inicial de preço isolada não são conversão de negócio.
