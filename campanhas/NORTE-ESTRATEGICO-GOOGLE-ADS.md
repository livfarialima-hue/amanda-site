...
**Estrutura e destinos:** manter intenção de procedimento em `AG_MASTOPEXIA` → `/mastopexia/`, `AG_MAMOPLASTIA_REDUTORA` → `/mamoplastia-redutora/`, `AG_PROTESE_MAMA` → `/protese-de-mama/`, `AG_ABDOMINOPLASTIA` → `/abdominoplastia/` e `AG_LIPOASPIRACAO` → `/lipoaspiracao/`. Desde 12/09/2026, `AG_CIRURGIA_MAMA_PRECO` → `/conteudos/quanto-custa-cirurgia-plastica-mama-sao-paulo/` e `AG_CONTORNO_CORPORAL_PRECO` → `/conteudos/quanto-custa-cirurgia-plastica-corporal-sao-paulo/` estão ativos. Os dois grupos concentram oito palavras de preço/valor com a correspondência preservada; as cinco cópias de frase permanecem pausadas na origem, as três exatas conflitantes foram removidas e seis negativas exatas de roteamento foram verificadas nos grupos de origem. Os dois novos RSAs estão ativos e `Qualificados`; a força `Ruim` é diagnóstico criativo, não falha de veiculação.
...
**Mensuração:** `{_camp}` mantém `G26MAMA` ou `G26CORP`; `{_ag}` distingue os sete grupos publicados. Marcações `ag_cirurgia_mama_preco` e `ag_contorno_corporal_preco` resolvem intenção de preço e landing, mas não inventam um procedimento específico. O registro foi publicado no deployment canônico do Apps Script v148. Marcação automática e `GCLID`/`GBRAID`/`WBRAID` permanecem preservados. A jornada first-party registra campanha, grupo, landing e CTA, e LEADS/CRM resolve apenas códigos e rotas exatas. Lead qualificado só entra na importação padrão com manifestação pessoal, fase canônica e exatamente um click ID elegível; clique, prefill e pergunta inicial de preço isolada não são conversão de negócio. A conexão diária `LEADS` havia concluído 6/6 linhas sem erro em 12/09; a inclusão dos grupos não altera esse critério.
...
**Recibo de ativação:** uploads de campanhas `8304615862134805016`, grupos `1035226498139761048`, palavras conformes `4941378120393558080`, RSAs `6913781089229317932`, sitelinks `1668147931032527165` e ativação `5196337282830450399`, todos aplicados sem erro. Cinco grupos, 13 palavras exatas/frase, cinco RSAs e 20 sitelinks ficaram ativos; políticas e aprendizado ainda podem mudar o estado individual de cada entidade. A prévia inicial encontrou termos bloqueados por política de saúde, que foram removidos sem pedido de exceção. O roteamento posterior adicionou dois grupos, oito palavras e dois RSAs de preço; removeu três exatas conflitantes pelo upload `6838173976875543099`; e completou as seis negativas exatas pelo upload `6049664068878736920`. A mensuração está materializada na v148 do Apps Script, com 6/6 qualificados reconciliados por `Opportunity ID` e importação diária do Google Ads concluída com 6 linhas e 0 erros. A rotina somente leitura `12117745` foi atualizada para R$ 103/dia e para os novos grupos, coincide com o SHA-256 `e6221436e176125d22ffdb48f28f1ea06689a8a324225c4c885556a16a59401f` e preserva a agenda diária entre 09:00 e 10:00.
