...
- **Decisão vigente para Meta Ads:** manter as campanhas faciais `M26F01W` e `M26F02S` com controle efetivo de **40 anos ou mais**. Não reduzir esse piso nem permitir expansão etária automática. Em 16 de agosto de 2026, a conferência ao vivo provou que o rótulo `40+` era somente sugestão do Público Advantage+: ambos os conjuntos tinham `age_min=25` como controle real. `M26F02S` foi convertido para público original e publicado com limite rígido `40–65+`. A mesma correção de `M26F01W` ficou bloqueada antes da publicação porque a Meta passou a exigir Conta do WhatsApp Business (`#2923012`) e orçamento total mínimo de `R$ 600,18`, contra `R$ 600,00`; o rascunho inviável foi descartado e o conjunto em produção não foi alterado.
- **Gate Meta WhatsApp 40+:** antes de estender, reativar ou editar `M26F01W`, decidir e executar de forma controlada a migração/conexão do número para a Conta do WhatsApp Business, confirmar preservação de conversas, mídia e contatos, corrigir somente o mínimo orçamentário exigido se a campanha ainda for usada e publicar o limite rígido `40–65+`. Até esse gate, o alerta diário permanece P0 e o nome `40+` não pode ser usado como prova de entrega. **Métrica:** `age_min` da API e distribuição real por idade; contatos válidos, qualificados e consultas por faixa quando mensuráveis. **Revisão do Site:** 24 horas, 7 dias e 14 dias após 16/08. **Manter:** limite rígido 40+ sem perda material de qualidade/volume elegível. **Reverter:** somente por decisão estratégica explícita documentada neste Norte, nunca por recomendação automática da Meta.
...
- **Decisão revisada após inspeção do criativo:** manter `M26F01W` somente com `C06H01` e destino WhatsApp; não renovar `M26F02S`; interromper `M26O01W`; e restringir o experimento de destino ao lifting cervical em dois braços: `M26C01W` — WhatsApp direto — e `M26C02S` — site → WhatsApp. Os dois estão preparados como campanhas de Tráfego, usam `C07H01`, arquivo 1:1 no Feed, o arquivo 9:16 enviado por Daniel em Reels/Stories, São Paulo +20 km, limite rígido 40–65+, todos os gêneros, posicionamentos Advantage+, orçamento total de R$ 300 por braço e janela de 17/08/2026 às 12h a 01/09/2026 às 12h. A rota determina também a meta operacional de entrega — conversas no braço direto e LPV no Site —, por isso a comparação mede qual sistema completo produz mais resultados de negócio e não causalidade pura do clique. `M26F01W/C06H01` corre em paralelo e é analisada separadamente; não é um terceiro braço nem um controle causal do cervical.
...
- **Gate de atribuição:** `age_min=40` observado nos dois braços cervicais; `M26C01W-C07H01` e `M26C02S-C07H01` preservados; `M26C02S` entra em `/lifting-cervical/` com UTMs canônicas; modo rico, schema, privacidade, purge e migração passam pelos gates próprios; sonda sintética sem paciente comprova Meta → site → navegação → CTA → WhatsApp → webhook → LEADS → CRM, com campanha, criativo, landing, página do CTA, caminho, confiança e first touch corretos; campanhas reconhecidas pelo agregado/rotina; Meta Ad IDs `120251248762170627` e `120251249058760627` mapeados; baseline e rollback salvos. Se qualquer gate falhar, adiar os dois braços.
...
- **Limitação etária do lifting facial:** o conjunto atual de `M26F01W` usa Advantage+ e mostra 40–65+ como sugestão, enquanto o controle mínimo observado permanece 25. Antes de aprovar a extensão, aceitar conscientemente essa exceção temporária ou adiar a renovação para reconstruí-la com piso rígido; não registrar esse conjunto como 40+ comprovado.
