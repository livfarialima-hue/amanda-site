# Norte estratégico de aquisição e conversão — Google Ads

**Status:** fonte canônica da estratégia
...
Este é o documento principal para todas as decisões estratégicas de Google Ads e da jornada de conversão que começa no Google e termina em consulta ou cirurgia.

Sempre que houver mudança de posicionamento, público, objetivo, orçamento total, estrutura de campanhas, conversão principal, estratégia de lances, critério de qualificação, página de destino, abordagem comercial ou regra de avaliação, este documento deve ser atualizado no mesmo dia.
...
Os demais documentos do projeto são manuais técnicos, planos de implementação ou históricos. Em caso de divergência estratégica, este documento prevalece. Eles só precisam ser alterados quando a mudança também afetar sua execução específica.
...
1. Clique de pesquisa com intenção compatível.
...
O orçamento total informado para Google Ads é de aproximadamente **R$ 2.800 por mês**. Esse número deve ser confirmado e atualizado aqui sempre que mudar.
...
Enquanto a mensuração offline e o volume de conversões qualificadas não estiverem suficientemente estáveis, a prioridade é **Google Pesquisa**. Performance Max, Demand Gen ou expansão automatizada não devem ser adotadas apenas para obter mais volume.
...
Qualquer nova campanha deve ser acrescentada a esta tabela antes ou no momento da ativação.
...
## 13. Diagnóstico e prioridade atuais

### Hipótese principal

O Google já encontra alguma procura compatível, mas ainda não está comprovado que:

1. compra os melhores termos com eficiência;
2. usa o lead qualificado como sinal real de lances;
3. produz confiança e valor percebido suficientes para a consulta;
4. recebe volume bastante para automação de lances robusta.

### Ordem de prioridade

1. Auditar ações de conversão, metas das campanhas, importação e taxa de correspondência dos identificadores.
2. Relacionar termos de pesquisa a leads e qualificados, adicionando negativas e preservando temas de alta intenção.
3. Verificar perda por orçamento e ranking nas pesquisas mais relevantes.
4. Testar mensagem de anúncio e página com uma hipótese por vez.
5. Medir a passagem de qualificado para consulta após as melhorias recentes do site e do WhatsApp.
6. Só então decidir sobre Maximizar conversões, CPA desejado ou expansão de canal.

## 14. Perguntas em aberto para a reunião com o Google
...
- Qual experimento único deve ser executado nos próximos 30 dias?

## 15. Protocolo para futuras alterações

Antes da mudança, acrescentar uma entrada ao histórico abaixo com status `Planejada`. Depois da execução, preencher a data, responsável e estado `Em observação`. Ao final da janela, registrar o resultado e marcar `Mantida`, `Revertida` ou `Substituída`.
