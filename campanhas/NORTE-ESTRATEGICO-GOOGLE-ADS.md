...
- Em 15 de agosto de 2026 foi publicado em modo **default-off** um contrato first-party de jornada: `J0` no navegador, `J1` de transporte com resgate em até dez minutos, `J2` durável e claim `C1` HMAC com expiração absoluta de 30 dias. O desenho separa first touch, conversa atual e último toque não direto, não classifica ausência de referrer como acesso direto e remove click IDs do texto visível quando o modo rico estiver ativo. Daniel aceitou formalmente o risco residual de a linha técnica `JID` ser visível, editável e encaminhável antes do primeiro resgate, inclusive a possibilidade de perda da atribuição se ela for removida e de atribuição incorreta ao destinatário se um encaminhamento for resgatado primeiro. Essa aceitação elimina a exigência de um transportador oculto como condição isolada, mas não autoriza ativação: `attributionJourneyEnabled` permanece `false`, o schema permanece desligado e ainda são obrigatórios política de privacidade atualizada, purge observado, dry-run/migração autorizados, reconciliação LEADS–CRM e sonda controlada. **Métrica:** taxa de resolução/fallback, divergência LEADS–CRM, encaminhamento ou atribuição incorreta e perda de JID. **Revisão:** 24 horas, 7 dias e 14 dias após eventual ativação. **Manter:** somente com zero PII, zero persistência indevida de JID, zero duplicidade e nenhuma atribuição incorreta comprovada. **Reverter:** desligar imediatamente a feature e voltar ao caminho legado diante de JID persistido fora do resolvedor, encaminhamento atribuído à pessoa errada, first touch sobrescrito, duplicidade ou regressão de atendimento.
...
Daniel autorizou a preparação do ciclo em 16/08/2026. Os objetos e o pacote técnico ficam em rascunho até a aprovação final prevista para 17/08; gasto continua proibido antes da publicação coordenada e dos gates abaixo.
...
- **Decisão revisada após inspeção do criativo:** manter `M26F01W` somente com `C06H01` e destino WhatsApp; não renovar `M26F02S`; interromper `M26O01W`; e restringir o experimento de destino ao lifting cervical em dois braços: `M26C01W` — WhatsApp direto — e `M26C02S` — site → WhatsApp. Os três novos objetos estão preparados como campanhas de Tráfego, com São Paulo +20 km, limite rígido 40–65+, todos os gêneros e orçamento total de R$ 300 por campanha. Os dois braços cervicais usam `C07H01`, arquivo 1:1 no Feed e arquivo 9:16 próprio em Reels/Stories, de 17/08/2026 às 12h a 01/09/2026 às 12h. O facial usa apenas o vencedor `C06H01`, também em 1:1 e 9:16, na mesma janela preparada. A rota determina a meta operacional de entrega — conversas no braço direto e LPV no Site —, por isso a comparação cervical mede qual sistema completo produz mais resultados de negócio e não causalidade pura do clique. `M26F01W/C06H01` corre em paralelo e é analisada separadamente; não é um terceiro braço nem um controle causal do cervical.
...
- **Orçamento:** o teste cervical soma R$ 600 em 15 dias, R$ 300 por braço. A continuidade de lifting facial adiciona R$ 300 no rascunho, levando o pacote a R$ 900. Qualquer valor superior deve ser apresentado e autorizado separadamente, sem aumento silencioso.
- **Reconstrução etária do lifting facial:** a extensão do objeto histórico de `M26F01W` foi descartada. O novo rascunho usa campanha `120251254720690627`, conjunto `120251254720700627`, anúncio principal `120251254720680627` e anúncio herdado desligado `120251254720710627`. O conjunto mostra controle rígido `40–65+` e São Paulo +20 km. Isso prova a configuração do rascunho, não entrega real; `age_min=40` e o estado ativo precisam ser reconfirmados depois da publicação seletiva.
