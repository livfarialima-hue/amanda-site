...
**Estrutura e destinos:** manter intenção de procedimento em `AG_MASTOPEXIA` → `/mastopexia/`, `AG_MAMOPLASTIA_REDUTORA` → `/mamoplastia-redutora/`, `AG_PROTESE_MAMA` → `/protese-de-mama/`, `AG_ABDOMINOPLASTIA` → `/abdominoplastia/` e `AG_LIPOASPIRACAO` → `/lipoaspiracao/`. A releitura ao vivo encontrou também `AG_CIRURGIA_MAMA_PRECO` e `AG_CONTORNO_CORPORAL_PRECO`, aplicados em paralelo com as oito palavras de preço/valor e seis negativas exatas nos grupos de origem. Ambos permanecem sem impressão, clique ou gasto, mas misturam procedimentos e têm RSA `Ruim`. O refinamento cria `AG_MASTOPEXIA_PRECO` e `AG_PROTESE_MAMA_PRECO` → `/conteudos/quanto-custa-cirurgia-plastica-mama-sao-paulo/` e `AG_LIPOASPIRACAO_PRECO` → `/conteudos/quanto-custa-cirurgia-plastica-corporal-sao-paulo/`; depois de recriar e conferir as oito palavras, pausa os dois grupos genéricos sem os excluir. A divisão por procedimento mantém cada RSA coerente com suas palavras e permite leitura precisa do funil. Até existir o novo recibo, sete grupos secundários continuam ativos e os três específicos não devem ser declarados como publicados.
...
**Qualidade e relevância dos anúncios:** a releitura viva de 12/09 encontrou `AG_MASTOPEXIA` e `AG_ABDOMINOPLASTIA` como `Ruim`, ambos sem impressão, clique ou gasto. O editor mostrou 15 títulos, mas somente três das quatro descrições pretendidas, e baixa presença das palavras ativas. Corrigir os dois RSAs antes da primeira entrega: mastopexia passa a usar naturalmente `lifting mamário`; abdominoplastia passa a usar `excesso de pele no abdômen` e `cirurgia de contorno abdominal`; os três novos RSAs de preço repetem as expressões específicas de seu próprio grupo. Preservar URL, caminhos, atribuição, promessa médica e ausência de faixa cirúrgica. A força exibida pelo Google continua sendo diagnóstico, não objetivo de negócio.
...
**Mensuração:** `{_camp}` mantém `G26MAMA` ou `G26CORP`; o registro reconhece dez códigos secundários durante a transição. `ag_cirurgia_mama_preco` e `ag_contorno_corporal_preco` ficam legíveis para preservar o intervalo da v148, sem inventar procedimento. Depois da migração, oito grupos permanecem ativos e `ag_mastopexia_preco`, `ag_protese_mama_preco` e `ag_lipoaspiracao_preco` preservam procedimento, intenção e landing. Marcação automática e `GCLID`/`GBRAID`/`WBRAID` permanecem intactos. Lead qualificado só entra na importação padrão com manifestação pessoal, fase canônica e exatamente um click ID elegível; clique, prefill e pergunta inicial de preço isolada não são conversão de negócio.
