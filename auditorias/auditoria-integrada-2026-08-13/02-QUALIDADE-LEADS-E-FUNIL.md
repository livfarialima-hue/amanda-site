...
**Atualização pós-auditoria — 14/08/2026:** o bloco `DAT-08` foi executado com backup e identidade exata. As 5 transações passaram a usar `Lead qualificado GCLID`, o ledger ficou em 5/5 após reconstruir 3 registros ausentes e o pós-voo terminou com zero divergência, duplicidade, ausência, linha inválida ou revisão. A conexão do Google Ads concluiu a importação manual das 5 linhas com 0 erros. O achado técnico está corrigido; o gate gerencial continua aberto até aceite/rejeição reconciliados e sete dias com a ação saudável.
...
**Checkpoint pós-auditoria — 14/08/2026:** `DAT-07` foi implementado prospectivamente no commit `7480022` e publicado no Apps Script versão 82/Netlify. Novos eventos passam a registrar categoria da referência e motivo explícito de fallback; a sonda M26F02S concluiu com `meta_attribution_contract_ok`. O achado histórico permanece: não há base para atribuir retroativamente as 1.290 LPVs nem para declarar cobertura ≥95% antes de uma amostra real.
