...
O alerta por e-mail da madrugada deve trazer a mensagem mais recente, indicar se a paciente já recebeu a confirmação curta e oferecer uma resposta contextual pronta para a manhã. A frase genérica `Recebi sua mensagem. Vou conferir essa informação com a equipe e retorno por aqui assim que possível.` não é uma sugestão humana válida e não deve aparecer nesse fluxo.
