...
1. começar por `Fila`, `Prioridade` e `Prazo`, usando `Abrir WhatsApp` para ler o histórico completo;
...
6. abrir `Central LIV > Processar decisões marcadas`, conferir as quantidades e confirmar uma única vez.
...
A marcação da caixa, isoladamente, nunca envia mensagem nem cancela plano. Se as duas decisões forem marcadas na mesma linha, o lote inteiro é bloqueado até a correção. O comando processa somente retomadas ainda elegíveis; linhas marcadas por engano permanecem humanas e recebem uma observação com o motivo. As aprovadas passam a `Bruna/bot` e `Automático`, preservam o horário planejado e continuam sujeitas à validação final individual. O cancelamento retira somente o plano selecionado e não marca `Nunca retomar`.

O e-mail diário inclui um botão direto para essa aba e cada linha tem `Abrir WhatsApp`, sem texto pré-preenchido e sem disparo automático. Linhas vencidas ficam vermelhas; `Resposta agora`, laranja; ação humana, amarela; retomada manual das próximas 24 horas, roxa; automação, verde; e `Aguardando paciente`, cinza. A coluna `Elegibilidade da Bruna` explica por que uma caixa não está disponível; falta de texto seguro aparece como `SEM SUGESTÃO PRONTA`, nunca como célula vazia. Esta ampliação está publicada no Apps Script v122 desde 23/08/2026.
