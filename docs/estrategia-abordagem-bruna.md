...
**Estado do release:** publicada e verificada em 20/08/2026. A confirmação humana de uma consulta pode registrar um horário negociado mesmo fora de `Datas Consulta`; a grade continua limitando somente a oferta e a confirmação automáticas. Para a Dra. Amanda, o atendimento permanece na `Sala 1`; conflito real gera alerta por e-mail, mas não cancela a decisão humana. O comprovante completo grava a linha de `Consultas` de forma atômica, atualiza LEADS/CRM, bloqueia a agenda e deixa o lembrete elegível. Comprovante incompleto não cria linha parcial nem evento: exige ação humana clara. Código funcional `27f07856e43cf90f898132ddf11913210818f2c4`, deploy Netlify `6a879160d29a140008a20503` e Apps Script v108 no deployment canônico preservado; **932/932 testes**, build de 178 arquivos, 44 URLs e `git diff --check` aprovados. O Netlify confirmou estado `ready`; o web app respondeu HTTP 200 e `Code.gs`/`ConsultasSync.gs` foram relidos com SHA-256 equivalente ao local. O caso operacional autorizado foi reconciliado em `Consultas`, LEADS, CRM e Sala 1, sem mensagem à paciente; a rotina diária de lembretes permanece instalada e o registro ficou elegível. As mesmas projeções deste manual e do Plano Executivo foram substituídas no Drive. Nenhuma campanha foi alterada. Rollback: deploy `6a8762898c14302d7062b1f9`, commit `46f7c91ea5cc00c2181fd3f7d564f61a880cdb11` e Apps Script v107.
