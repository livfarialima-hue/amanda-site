...
**Último release publicado:** verificado em 23/08/2026. Toda primeira retomada programada passa por uma leitura semântica dos 20 turnos recentes imediatamente antes do provedor. A segunda e última retomada somente reutiliza essa decisão quando o silêncio, a identidade da mensagem anterior e a ausência de qualquer fala posterior estão comprovados; caso contrário, chama a IA novamente e falha fechada. A IA apenas confirma `send_exact` com `context_aligned` e confiança alta ou veta para revisão; nunca reescreve silenciosamente a mensagem aprovada nem substitui as travas determinísticas. A Central se reorganiza depois do processamento, mantém o histórico terminal recente por 24 horas, processa cancelamentos marcados na atualização seguinte e, ao marcar `Aprovar com a Bruna`, sugere uma próxima janela com melhor resposta observada sem enviar. Quando a paciente responde a uma retomada com uma pergunta segura, a fala passa a ser um novo turno ativo e a Bruna pode continuar o assunto sem reapresentação; agenda, preço fora da regra, cuidado clínico, takeover e procedimento incerto continuam humanos. Código funcional da Bruna `dc3d6054e895968e98f369e39a3905fca5559227`, deploy Netlify `6a8b399c98d6cf000824079f` e Apps Script v126 no deployment canônico preservado; **44/44 testes da Central**, **96/96 testes focados da mudança** e **1016/1016 testes integrais**. O web app respondeu HTTP 200 e o token inválido não cancelou retomada nem mudou preferência. Nenhuma função, trigger ou mensagem real foi executada. Rollback operacional da Central: Apps Script v125.
