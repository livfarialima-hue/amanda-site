...
**Decisão local candidata — pedido direto de Instagram:** quando a mensagem atual pedir Instagram, Insta, perfil oficial, rede social ou arroba da Dra. Amanda, a resposta se limita ao perfil oficial `https://www.instagram.com/dra.amanda_plastica/`. A presença de `Ref.`, código ou origem de anúncio em qualquer mensagem anterior não autoriza explicar rastreamento, campanha ou código interno. Essa explicação permanece disponível somente diante de pergunta explícita da paciente sobre o significado da referência. Esta decisão está implementada apenas como candidata local e ainda não representa publicação.

**Decisão local candidata — acolhimento pediátrico antes do handoff:** quando a mensagem mencionar explicitamente uma criança, bebê, adolescente ou menor, o caso continua em risco vermelho, sem orientação clínica automática e com alerta humano obrigatório. Depois de criar o encaminhamento interno, a Bruna pode enviar uma única ciência curta: reconhece que se trata de uma criança e informa que a equipe responsável confirmará a orientação adequada para o caso. No primeiro contato sem nome pessoal confiável, termina com `Como posso te chamar?`; com nome válido ou histórico anterior, não pergunta novamente. A mensagem não cita procedimento por suposição, não explica indicação, técnica, prazo, preço ou agenda e não é enviada diante de urgência, takeover, opt-out, duplicidade ou fala mais recente. Esta decisão está implementada apenas como candidata local e ainda não representa publicação.
...
Em pedido direto de Instagram, responder somente com o perfil oficial. Não aproveitar a presença de uma referência de anúncio no histórico para explicar códigos internos, atribuição ou campanha. A explicação da referência é uma intenção separada e só deve aparecer quando a própria paciente perguntar por ela na mensagem atual.
