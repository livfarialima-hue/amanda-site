...
**Mudança candidata — ainda não publicada:** toda retomada programada passa por uma nova leitura semântica dos 20 turnos recentes imediatamente antes do provedor. A IA apenas confirma `send_exact` com alta confiança ou veta para revisão; nunca reescreve silenciosamente a mensagem aprovada, nunca substitui as travas determinísticas e falha fechada quando o contexto ou a própria análise estiverem indisponíveis.
