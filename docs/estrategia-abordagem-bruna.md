...
**Estado do release:** publicada e verificada em 20–21/08/2026. A classificação usa contexto pessoal posterior ao prefill: texto automático, referência de campanha ou pergunta inicial de preço isolada não bastam para qualificar, enviar conversão offline ou concluir prontidão para agenda. A referência Meta canônica só completa a origem quando o provedor omite `source_id`; expressões soltas nunca substituem identificador estruturado. A auditoria de 54 exportações, correspondentes a 53 conversas únicas, não encontrou conversa identificável de aquisição ausente da LEADS. Foram reconciliadas cinco fases — quatro para `Novo` e uma para `Consulta agendada` —, arquivada uma conversa interna e invalidadas duas conversões qualificadas falsas, removidas também da fila `IMPORT_GOOGLE_ADS`. A conexão do Google registrava última atualização em 15/08/2026, portanto esses dois eventos de 18–19/08 não chegaram à conta. Código funcional `dd93f1cebf5c218672d1bf84d6d3b39ca3c9ff74`, deploy Netlify `6a879f71c5efe0ad12609a3d`, correção de saneamento `88cdbd0` e Apps Script v110 no deployment canônico preservado; **940/940 testes**, build de 178 arquivos e 44 URLs aprovados. Nenhuma campanha, orçamento ou lance foi alterado. Rollback de código: deploy `6a879160d29a140008a20503`, commit `27f07856e43cf90f898132ddf11913210818f2c4` e Apps Script v109; o saneamento auditado da planilha não deve ser revertido automaticamente.
...
Se um evento declarado como mensagem de texto chegar sem corpo legível, mesmo depois de verificar os formatos seguros conhecidos do provedor, não existe conteúdo para a IA interpretar. Nesse caso técnico e somente nele, a Bruna não permanece em silêncio nem tenta adivinhar campanha, procedimento ou intenção: envia uma única pergunta neutra — `Olá! Eu sou a Bruna, concierge da Clínica LIV Faria Lima. Recebi seu contato, mas a mensagem não apareceu completa para mim. Pode me contar qual procedimento ou dúvida você gostaria de entender primeiro?` A resposta respeita mensagem mais recente, duplicidade, preferência de contato, horário extremo e intervenção humana. O sistema não fabrica nem registra uma fala da paciente que não recebeu.
