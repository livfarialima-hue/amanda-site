...
**Estado geral:** produção contextual `2026-08-19.4` publicada e verificada. A candidata local `2026-08-19.5` autoriza a faixa de otoplastia de R$ 8 mil a R$ 14 mil somente no segundo passo, com ressalvas e bloqueio de repetição; está validada em **896/896 testes** e aguarda autorização explícita de publicação. O ledger durável sustenta o contexto mesmo se a memória auxiliar falhar. O incidente de integridade foi corrigido nas versões 102/103 e a última divergência funcional do Apps Script foi encerrada na v104 com a sincronização autorizada de `CentralAtendimento.gs`. Os 22 arquivos do Apps Script estão funcionalmente alinhados ao repositório; as quatro diferenças remanescentes são somente ausência de quebra de linha final e não alteram execução. Nenhuma mensagem real foi enviada. A projeção ativa do Drive continua representando `2026-08-19.4` e só deve ser substituída no fechamento do release autorizado.
...
- `2026-08-19.5` está `CANDIDATA LOCAL — PUBLICAÇÃO PENDENTE`: preserva o primeiro pedido de preço de otoplastia sem números e libera R$ 8 mil a R$ 14 mil uma única vez somente depois de aceite claro ou novo pedido explícito. Faixa divergente, ausência de ressalvas, repetição ou contexto humano bloqueiam o envio. Perguntas compostas de otoplastia são respondidas antes do preço; `otomodelação` não define técnica, duração ou indicação. O webhook completo da aceitação ao único envio e a retomada diferida passaram, assim como **896/896 testes**. Apps Script v104 preservado, nenhuma mensagem real enviada e nenhum Google Ads alterado;
