...
**Estado geral:** produção contextual `2026-08-21.1` publicada e verificada. As retomadas comerciais permanecem limitadas a 24h e 72h, encerram diante de pausa, recusa ou promessa de retorno e agora permitem cancelar somente o plano selecionado ou passar uma ação humana elegível à Bruna com confirmação e revalidação completa. Os textos de cuidados pós-consulta ficaram mais contextuais e sem pressão. Apps Script v111 no deployment canônico preservado, código funcional `6551f0fadd21f25ee238cc0fb903495ec7af6ce6`; **948/948 testes**, foco **41/41**, arquivos relidos por SHA-256 normalizado e web app HTTP 200. A auditoria anterior de 54 exportações e 53 conversas únicas continua encerrada: quatro oportunidades foram corrigidas para `Novo`, uma para `Consulta agendada`, uma conversa interna foi arquivada e duas conversões falsas foram invalidadas antes de importação. Nenhuma função, trigger, mensagem real, campanha, orçamento ou lance foi acionado neste pós-voo.

**Release `2026-08-22.1` — CANDIDATA LOCAL VALIDADA:** o incidente ao vivo foi entregue pela YCloud como `unsupported`, sem corpo textual ou referência utilizável, embora o aplicativo exibisse a mensagem. A correção preserva `route_pending` e revisão humana, mas envia uma única pergunta neutra para recuperar procedimento ou dúvida, com mensagem mais recente, deduplicação, opt-out, horário e takeover intactos. Não reconstrói texto, não presume anúncio, não cria oportunidade por inferência e registra somente códigos técnicos anônimos. Testes do Netlify **635/635**; nenhuma mensagem real enviada. A produção permanece em `2026-08-20.4` até a publicação do commit candidato.
...
**Projeção deste Plano Executivo no Drive:** o mesmo arquivo canônico remoto `18iUqY6HttJwPusSAA1VGmrMqqRluyjTO` foi substituído no fechamento da versão `2026-08-21.1`; nenhuma cópia concorrente foi criada.
