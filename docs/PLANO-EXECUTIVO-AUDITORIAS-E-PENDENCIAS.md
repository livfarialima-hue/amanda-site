...
**Estado geral:** produção contextual `2026-08-19.5` publicada e verificada. A candidata `2026-08-19.6` recupera eventos textuais cujo corpo não ficou disponível: tenta formatos seguros alternativos e, se ainda não houver texto, envia uma única pergunta neutra da Bruna, sem inferir procedimento, qualificação ou agenda. A correção passou em **899/899 testes** e aguarda o fechamento do release autorizado; nenhuma mensagem real foi enviada. O Apps Script permanece na v104 e nenhum Google Ads foi alterado. A projeção ativa do Drive continua representando `2026-08-19.5` até a publicação.

**Sincronização pendente — projeção deste Plano Executivo no Drive:** o arquivo canônico remoto `18iUqY6HttJwPusSAA1VGmrMqqRluyjTO` ainda representa a versão anterior. Em 19/08/2026, a substituição do mesmo arquivo foi bloqueada pela permissão de upload do cliente; nenhuma cópia concorrente foi criada. Repetir somente a substituição nesse mesmo ID quando a permissão for liberada e conferir o conteúdo contra o arquivo local.
...
- `2026-08-19.6` está `CANDIDATA LOCAL — PUBLICAÇÃO AUTORIZADA`: o incidente das 20:50:31 foi persistido como texto vazio e terminou em `route_pending`, sem oportunidade nem resposta automática. A correção lê envelopes textuais alternativos conhecidos e, quando o corpo realmente não existe, envia uma única pergunta neutra protegida por mensagem mais recente, deduplicação, preferência de contato, horário extremo e corrida humana; **899/899 testes** passaram e nenhuma mensagem real foi enviada;
