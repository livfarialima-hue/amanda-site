...
**Estado geral:** produção contextual `2026-08-20.1` publicada e verificada. Todo texto elegível já persistido recebe avaliação semântica mesmo antes de existir oportunidade ou rota resolvida; a avaliação orienta contexto e sugestão, mas não substitui autorização de envio. Alta confiança pode recuperar Amanda ou Daniel em modo ativo, ambiguidade segura recebe uma pergunta e casos protegidos permanecem com a equipe. `Shadow` não envia nem altera agenda; `off` preserva a entrada e bloqueia IA, mensagens, agenda e disparos programados. Código funcional `afa230263288bba88fb0cb61f4fb55e5903d4dca`, deploy Netlify `6a8701bb1ae7b60008c3a8ac` e Apps Script v106; **912/912 testes**, build de 178 arquivos e 44 URLs sem erro. Domínio, URL imutável, webhook em modo `active` e web app responderam HTTP 200. Nenhuma mensagem real foi enviada e nenhum Google Ads foi alterado. As mesmas projeções deste plano e do manual foram substituídas no Drive.

**Candidata `2026-08-20.2` — VALIDADA E AUTORIZADA PARA PUBLICAÇÃO:** responde diretamente à comparação geral entre minilifting e lifting facial; transforma uma revisão segura em ação idempotente com rascunho contextual para conferência na Central/e-mail; e substitui a oferta mecânica da primeira resposta de preço de lifting facial, cervicoplastia e otoplastia pela proposta leve de faixa geral no passo seguinte. A faixa numérica continua somente no aceite ou novo pedido explícito e os temas protegidos permanecem sem rascunho copiável. Validação local: **923/923 testes**, build de 178 arquivos, 44 URLs e diff aprovados. Commit, Netlify, Apps Script e substituição das mesmas projeções no Drive ainda precisam ser fechados; até lá: `sincronização pendente`.
...
- `BRUNA-CONTEXTO-PRECO-2026-08-20` está `CANDIDATA LOCAL VALIDADA E AUTORIZADA`: diferença geral entre minilifting e lifting facial coberta sem indicação individual, rascunho interno seguro ligado à revisão e oferta progressiva de faixa somente para lifting facial, cervicoplastia e otoplastia. Regressões do caso real, Central, e-mail, risco alto e fluxo de preço passaram em **923/923**; publicar e revisar as primeiras ocorrências reais, com leitura em sete dias;
