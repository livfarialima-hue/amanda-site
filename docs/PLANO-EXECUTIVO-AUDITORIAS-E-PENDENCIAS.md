...
**Atualizado em:** 20 de agosto de 2026, 20:52, America/Sao_Paulo
...
**Estado geral:** produção contextual `2026-08-20.3` publicada e verificada. A confirmação humana pode registrar horário fora de `Datas Consulta`; a grade limita somente oferta e confirmação automáticas. Amanda continua exclusivamente na `Sala 1`, com alerta por e-mail apenas quando existir conflito real e sem desfazer o agendamento humano. A escrita de `Consultas` é atômica, comprovante incompleto não deixa linha parcial e o timeout é reconciliado por releitura idempotente. Código funcional `27f07856e43cf90f898132ddf11913210818f2c4`, deploy Netlify `6a879160d29a140008a20503` e Apps Script v108; **932/932 testes**, build de 178 arquivos e 44 URLs sem erro. Netlify `ready` e web app HTTP 200; arquivos Apps Script relidos e equivalentes ao local. O caso operacional autorizado foi reconciliado em `Consultas`, LEADS, CRM e Sala 1, sem mensagem à paciente, e ficou elegível para a rotina diária de lembretes. Nenhum Google Ads foi alterado. As mesmas projeções deste plano e do manual foram substituídas no Drive.
...
**Projeção deste Plano Executivo no Drive:** o mesmo arquivo canônico remoto `18iUqY6HttJwPusSAA1VGmrMqqRluyjTO` foi substituído no fechamento da versão `2026-08-20.3`; nenhuma cópia concorrente foi criada.
